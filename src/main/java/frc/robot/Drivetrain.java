...
class Drivetrain {
  ...
  private final SwerveModule frontLeftModule = new SwerveModule(1, 2, 1, false, -0.165527, "canivore"); 
  private final SwerveModule frontRightModule = new SwerveModule(3, 4, 2, true, 0.100342 , "canivore");
  private final SwerveModule backRightModule = new SwerveModule(5, 6, 3, true, 0.161865 , "canivore");
  private final SwerveModule backLeftModule = new SwerveModule(7, 8, 4, false, 0.230469, "canivore");
  ...
  private SwerveModulePosition[] modulePositions = getModulePositions();
  ...
  public void addVisionEstimate(int limelightIndex) {
    ...
    if (botpose.tagCount >= 2 || botpose.avgTagArea*botpose.tagCount > 2.0) SD = 0.1; // Reduces the standard deviation when there are multiple tags in sight, or the tags are close to the camera.
    if (botpose.tagCount >= 1 && botpose.avgTagArea*botpose.tagCount > 0.2 && Math.sqrt(Math.pow(getXVel(), 2) + Math.pow(getYVel(), 2)) < 1.0 && getAngVel() < 90.0) { // >1 April Tag is detected, the robot is relatively close to the April Tags, and the robot is relatively stationary.
